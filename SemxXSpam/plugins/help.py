...
HELP_PIC = "https://telegra.ph/file/2c5b49667665d4372a668.jpg"

RyanHelp = "★ 𝙍𝙁𝙎 𝙁𝙄𝙂𝙃𝙏𝙀𝙍 𝙃𝙚𝙡𝙥 𝙈𝙚𝙣𝙪 ★\n𝐂𝐥𝐢𝐜𝐤 𝐎𝐧 𝐁𝐞𝐥𝐨𝐰 𝐁𝐮𝐭𝐭𝐨𝐧𝐬 𝐅𝐨𝐫 𝐇𝐞𝐥𝐩\n\n\n@TRINETRA_OWNER"
...
async def help(event):
    if event.sender_id in SUDO_USERS:
       await event.client.send_file(event.chat_id,
           ...
           [    
            Button.url("✨ ᴄʜᴀɴɴᴇʟ ✨", "https://t.me/RFS_BOTHUB")
           ],
           [
           Button.url("✨ sᴜᴘᴘᴏʀᴛ ✨", "https://t.me/RFS_SUPPORT")
           ],
           ],
           )              
...
async def helpback(event):
   if event.query.user_id in SUDO_USERS:    
      await event.edit(
        ...
        Alert = (
                "Noob !! Make Your Own Semc X Spam Bots !! @TRINETRA_OWNER"
            )
...
async def help_spam(event):
   if event.query.user_id in SUDO_USERS:    
       await event.edit(
        ...
        Alert = (
                "Noob !! Make Your Own RFS FIGHTER Bots !! @TRINETRA_OWNER"
            )
        await event.answer(Alert, cache_time=0, alert=True)
